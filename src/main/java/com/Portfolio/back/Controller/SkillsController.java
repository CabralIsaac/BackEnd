...
import com.Portfolio.back.Service.ISkillsService;
...
@CrossOrigin(origins = "https://frontend-ffd02.web.app/")
...
public class SkillsController {
    ...
    public ResponseEntity<?> mostrarSkillporId(@PathVariable int id) {
        ...
        if (skill == null) {
            return new ResponseEntity<>("No se encontro la Skill", HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity<?> agregarSkilll(@RequestBody Skills skill) {
        if (StringUtils.isBlank(skill.getNombreSkill())
            ...
            return new ResponseEntity<>("Campos vacios o incorrectos.", HttpStatus.BAD_REQUEST);
    ...
    public ResponseEntity<?> borrarSkills(@PathVariable int id) {
        ...
        if (iSkillsService.traerSkillsPorId(id) == null) {
            return new ResponseEntity<>("No se encontro la skill", HttpStatus.NOT_FOUND);
        }
